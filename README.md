...
```bash
az k8s-configuration flux delete -g flux-demo-rg -c flux-demo-arc -n oesdevops-config --namespace oesdevops-config -t connectedClusters

az k8s-configuration flux create -g flux-demo-rg -c flux-demo-arc -n oesdevops-config --namespace oesdevops-config -t connectedClusters --scope cluster -u https://github.com/Azure/gitops-flux2-kustomize-helm-mt --branch main  --kustomization name=infra path=./infrastructure prune=true --kustomization name=apps path=./apps/staging prune=true dependsOn=["infra"]
```
